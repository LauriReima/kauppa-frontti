        ...
        return (
            <>
                <div className='cardDiv' style={styles.card} >
                    ...
                    <br></br>
                    {user !== null && user.username === 'lauri' ? <button className='button-35' onClick={() => deleteP(id)}><i className="fa-regular fa-trash-can"></i></button> : ''}
                    {user ? <button className='button-35' onClick={() => addToCart(id)}><i className="fa-solid fa-cart-plus"></i></button> : ''}
                </div>
            </>
        )
}
